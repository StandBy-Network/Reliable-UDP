...
struct event_data {
    ...
    struct zts_timeval e_time; /* Timeout */
...
*/
int
event_timeout(struct zts_timeval t, int (*fn)(int, void*), void *arg, char *str) {
  ...
  while (ee || ee_timers) {
    ...
    if (ee_timers) {
      ...
      if (t.tv_sec < 0)
        ...
        n = zts_select(ZTS_FD_SETSIZE, &fdset, NULL, NULL, &t);
    }
    else
      n = zts_select(ZTS_FD_SETSIZE, &fdset, NULL, NULL, NULL);

    if (n == -1)
      if (zts_errno != ZTS_EINTR)
